...
class VenueBuilder:
    ...
    def select_attributes(self, venue_details):
        ...
        foursquare_id, name, price, rating = venue_details['id'], venue_details['name'], venue_details['price']['tier'], venue_details.get('rating', -99)
        likes = venue_details.get('likes', {}).get('count', -99)
...
class LocationBuilder:
    ...
    def find_or_create_by_city_state_zip(self, city_name = 'N/A', state_name = 'N/A', code = -99, conn = None, cursor = None):
    ...
    def build_location_city_state_zip(self, location_attr, conn, cursor):
        ...
        code = location_attr.pop('postalCode', -99)
        city, state, zipcode = self.find_or_create_by_city_state_zip(city_name, state_name, code, conn, cursor)
        location = models.Location(latitude = location_attr['lat'],
                longitude = location_attr['lng'],
                address = location_attr['address'],
